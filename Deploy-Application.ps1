...
Try {
	...
	[string]$appScriptDate = '02/26/2019'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		#uninstall first (keep if updating to a later version so you can unistall the old before installing the new)
		if (Test-Path -Path "$envProgramFilesX86\LiveHelpNow\LiveHelpNow.exe") {
			Execute-MSI -Action Uninstall -Path '{030CE529-FBD6-4FD7-96F9-4674A2EEC503}'
		}
		...
		Execute-MSI -Action 'Install' -Path "$dirFiles\LiveHelpNow.msi" -Parameters 'REBOOT=ReallySuppress /QB'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Execute-MSI -Action Uninstall -Path '{7F6BBAAD-DF3F-42B6-9C6B-222CD6E10A63}'
